...
def latent_factor_recommend(review, id):
    ...
    return pd.DataFrame(
        svd_user_predicted_ratings, columns=df_user_beer_ratings.columns), df_userid_to_rownum[df_userid_to_rownum['id'] == id].index[0]


def recommend_beers(row_num, df_svd_preds, id, ori_beer_df, ori_ratings_df, num_recommendations=5):

    # userNumber를 index로 나타내기
    # user_row_number = id - 1

    # 최종적으로 만든 pred_df에서 사용자 index에 따라 맥주 데이터 정렬 -> 맥주 평점이 높은 순으로 정렬
    sorted_user_predictions = df_svd_preds.loc[row_num].sort_values(
        ascending=False)
    ...
    recommendations = recommendations.rename(
        columns={row_num: 'Predictions'}).sort_values('Predictions', ascending=False).head(num_recommendations)
...
def use_latent(id):
    ...
    alredady_rated, predictions = recommend_beers(
        row_num, df_svd_preds, id, beer, review, 10)
